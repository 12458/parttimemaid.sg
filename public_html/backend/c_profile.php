      ...
      <div class="row">
		<div class="col-md-12">
		  <form name="rzvy_profile_form" id="rzvy_profile_form" method="post">
			  <input type='hidden' id="rzvy-profile-admin-id-hidden" name="rzvy-profile-admin-id-hidden" value="<?php echo $_SESSION['customer_id']; ?>" />
			  ...
			  <div class="form-group row">
				...
				<div class="col-md-4">
					<label class="control-label"><?php if(isset($rzvy_translangArr['birthdate'])){ echo $rzvy_translangArr['birthdate']; }else{ echo $rzvy_defaultlang['birthdate']; } ?></label>
					  <div class="input-group">
						<input class="form-control" id="rzvy_profile_dob" name="rzvy_profile_dob" type="text" value="<?php if($obj_settings->get_option('rzvy_birthdate_with_year') == "Y"){ echo date("j F Y", strtotime($profile_data['dob'])); }else{ echo date("j F", strtotime($profile_data['dob'])); } ?>" onfocus="this.blur()" />
						<div class="input-group-append">
						  <span class="input-group-text"><i class="fa fa-calendar"></i></span>
						</div>
					  </div>
						<label for="rzvy_profile_dob" generated="true" class="error"></label>
				  </div>
			  </div>
			  <a class="btn btn-primary btn-block rzvy_update_profile_btn" href="javascript:void(0);"><?php if(isset($rzvy_translangArr['update_profile'])){ echo $rzvy_translangArr['update_profile']; }else{ echo $rzvy_defaultlang['update_profile']; } ?></a>
		 </form>
       </div>
		</div>
					...
					<form name="rzvy_change_profile_email_form" id="rzvy_change_profile_email_form" method="post">
						<div class="row m-2">
							...
							<div class="col-md-3 p-1">
								<a href="javascript:void(0)" class="btn btn-primary w-100" id="rzvy_change_profile_email_btn"><?php if(isset($rzvy_translangArr['change'])){ echo $rzvy_translangArr['change']; }else{ echo $rzvy_defaultlang['change']; } ?></a>
							</div>
						</div>
					</form>
				</div>
