...
if (!class_exists('Rzvy_Hooks')){
  ...
  */
  class Rzvy_Hooks
  {
	...
	var $adminFooter = array();
	var $staffHeader = array();
	var $staffFooter = array();
	var $customerHeader = array();
	var $customerFooter = array();
	var $onepageHeader = array();
	var $onepageFooter = array();
	var $stepviewHeader = array();
	var $stepviewFooter = array();
	var $registerHeader = array();
	var $registerFooter = array();
	var $loginHeader = array();
	var $loginFooter = array();
	var $forgetpwdHeader = array();
	var $forgetpwdFooter = array();
	var $resetpwdHeader = array();
	var $resetpwdFooter = array();
    /**
    ...
    public function adminHeaderIncludes() {
		if(sizeof($this->adminHeader)>0){
			$currentPage=explode("/",$_SERVER['SCRIPT_NAME']);
			$currentPageFile = end($currentPage);
			foreach($this->adminHeader as $script ){
				if($script["type"] == "css" && (in_array("all", $script["page"]) || in_array($currentPageFile, $script["page"]))){
					?><link rel="stylesheet" href="<?php echo $script["url"]; ?>"><?php 
				} else if($script["type"] == "js" && (in_array("all", $script["page"]) || in_array($currentPageFile, $script["page"]))){
					?><script src="<?php echo $script["url"]; ?>"></script><?php 
				} else if($script["type"] == "other" && (in_array("all", $script["page"]) || in_array($currentPageFile, $script["page"]))){ $this->do_action($script["action"]); } else {}
			}
		}
    }
    public function adminFooterIncludes() {
		if(sizeof($this->adminFooter)>0){
			$currentPage=explode("/",$_SERVER['SCRIPT_NAME']);
			$currentPageFile = end($currentPage);
			foreach($this->adminFooter as $script ){
				if($script["type"] == "css" && (in_array("all", $script["page"]) || in_array($currentPageFile, $script["page"]))){
					?><link rel="stylesheet" href="<?php echo $script["url"]; ?>"><?php 
				} else if($script["type"] == "js" && (in_array("all", $script["page"]) || in_array($currentPageFile, $script["page"]))){
					?><script src="<?php echo $script["url"]; ?>"></script><?php 
    ...
    public function staffHeaderIncludes() {
		if(sizeof($this->staffHeader)>0){
			$currentPage=explode("/",$_SERVER['SCRIPT_NAME']);
			$currentPageFile = end($currentPage);
			foreach($this->staffHeader as $script ){
				if($script["type"] == "css" && (in_array("all", $script["page"]) || in_array($currentPageFile, $script["page"]))){
					?><link rel="stylesheet" href="<?php echo $script["url"]; ?>"><?php 
				} else if($script["type"] == "js" && (in_array("all", $script["page"]) || in_array($currentPageFile, $script["page"]))){
					?><script src="<?php echo $script["url"]; ?>"></script><?php 
    ...
    public function staffFooterIncludes() {
		if(sizeof($this->staffFooter)>0){
			$currentPage=explode("/",$_SERVER['SCRIPT_NAME']);
			$currentPageFile = end($currentPage);
			foreach($this->staffFooter as $script ){
				if($script["type"] == "css" && (in_array("all", $script["page"]) || in_array($currentPageFile, $script["page"]))){
					?><link rel="stylesheet" href="<?php echo $script["url"]; ?>"><?php 
				} else if($script["type"] == "js" && (in_array("all", $script["page"]) || in_array($currentPageFile, $script["page"]))){
					?><script src="<?php echo $script["url"]; ?>"></script><?php 
	...
	public function customerHeaderIncludes() {
		if(sizeof($this->customerHeader)>0){
			$currentPage=explode("/",$_SERVER['SCRIPT_NAME']);
			$currentPageFile = end($currentPage);
			foreach($this->customerHeader as $script ){
				if($script["type"] == "css" && (in_array("all", $script["page"]) || in_array($currentPageFile, $script["page"]))){
					?><link rel="stylesheet" href="<?php echo $script["url"]; ?>"><?php 
				} else if($script["type"] == "js" && (in_array("all", $script["page"]) || in_array($currentPageFile, $script["page"]))){
					?><script src="<?php echo $script["url"]; ?>"></script><?php 
    ...
    public function customerFooterIncludes() {
		if(sizeof($this->customerFooter)>0){
			$currentPage=explode("/",$_SERVER['SCRIPT_NAME']);
			$currentPageFile = end($currentPage);
			foreach($this->customerFooter as $script ){
				if($script["type"] == "css" && (in_array("all", $script["page"]) || in_array($currentPageFile, $script["page"]))){
					?><link rel="stylesheet" href="<?php echo $script["url"]; ?>"><?php 
				} else if($script["type"] == "js" && (in_array("all", $script["page"]) || in_array($currentPageFile, $script["page"]))){
					?><script src="<?php echo $script["url"]; ?>"></script><?php 
	...
	public function onepageHeaderIncludes() {
		if(sizeof($this->onepageHeader)>0){
			$currentPage=explode("/",$_SERVER['SCRIPT_NAME']);
			$currentPageFile = end($currentPage);
			foreach($this->onepageHeader as $script ){
				if($script["type"] == "css" && (in_array("all", $script["page"]) || in_array($currentPageFile, $script["page"]))){
					?><link rel="stylesheet" href="<?php echo $script["url"]; ?>"><?php 
				} else if($script["type"] == "js" && (in_array("all", $script["page"]) || in_array($currentPageFile, $script["page"]))){
					?><script src="<?php echo $script["url"]; ?>"></script><?php 
    ...
    public function onepageFooterIncludes() {
		if(sizeof($this->onepageFooter)>0){
			$currentPage=explode("/",$_SERVER['SCRIPT_NAME']);
			$currentPageFile = end($currentPage);
			foreach($this->onepageFooter as $script ){
				if($script["type"] == "css" && (in_array("all", $script["page"]) || in_array($currentPageFile, $script["page"]))){
					?><link rel="stylesheet" href="<?php echo $script["url"]; ?>"><?php 
				} else if($script["type"] == "js" && (in_array("all", $script["page"]) || in_array($currentPageFile, $script["page"]))){
					?><script src="<?php echo $script["url"]; ?>"></script><?php 
	...
	public function stepviewHeaderIncludes() {
		if(sizeof($this->stepviewHeader)>0){
			$currentPage=explode("/",$_SERVER['SCRIPT_NAME']);
			$currentPageFile = end($currentPage);
			foreach($this->stepviewHeader as $script ){
				if($script["type"] == "css" && (in_array("all", $script["page"]) || in_array($currentPageFile, $script["page"]))){
					?><link rel="stylesheet" href="<?php echo $script["url"]; ?>"><?php 
				} else if($script["type"] == "js" && (in_array("all", $script["page"]) || in_array($currentPageFile, $script["page"]))){
					?><script src="<?php echo $script["url"]; ?>"></script><?php 
    ...
    public function stepviewFooterIncludes() {
		if(sizeof($this->stepviewFooter)>0){
			$currentPage=explode("/",$_SERVER['SCRIPT_NAME']);
			$currentPageFile = end($currentPage);
			foreach($this->stepviewFooter as $script ){
				if($script["type"] == "css" && (in_array("all", $script["page"]) || in_array($currentPageFile, $script["page"]))){
					?><link rel="stylesheet" href="<?php echo $script["url"]; ?>"><?php 
				} else if($script["type"] == "js" && (in_array("all", $script["page"]) || in_array($currentPageFile, $script["page"]))){
					?><script src="<?php echo $script["url"]; ?>"></script><?php 
	...
	public function registerHeaderIncludes() {
		if(sizeof($this->registerHeader)>0){
			$currentPage=explode("/",$_SERVER['SCRIPT_NAME']);
			$currentPageFile = end($currentPage);
			foreach($this->registerHeader as $script ){
				if($script["type"] == "css" && (in_array("all", $script["page"]) || in_array($currentPageFile, $script["page"]))){
					?><link rel="stylesheet" href="<?php echo $script["url"]; ?>"><?php 
				} else if($script["type"] == "js" && (in_array("all", $script["page"]) || in_array($currentPageFile, $script["page"]))){
					?><script src="<?php echo $script["url"]; ?>"></script><?php 
    ...
    public function registerFooterIncludes() {
		if(sizeof($this->registerFooter)>0){
			$currentPage=explode("/",$_SERVER['SCRIPT_NAME']);
			$currentPageFile = end($currentPage);
			foreach($this->registerFooter as $script ){
				if($script["type"] == "css" && (in_array("all", $script["page"]) || in_array($currentPageFile, $script["page"]))){
					?><link rel="stylesheet" href="<?php echo $script["url"]; ?>"><?php 
				} else if($script["type"] == "js" && (in_array("all", $script["page"]) || in_array($currentPageFile, $script["page"]))){
					?><script src="<?php echo $script["url"]; ?>"></script><?php 
	...
	public function loginHeaderIncludes() {
		if(sizeof($this->loginHeader)>0){
			$currentPage=explode("/",$_SERVER['SCRIPT_NAME']);
			$currentPageFile = end($currentPage);
			foreach($this->loginHeader as $script ){
				if($script["type"] == "css" && (in_array("all", $script["page"]) || in_array($currentPageFile, $script["page"]))){
					?><link rel="stylesheet" href="<?php echo $script["url"]; ?>"><?php 
				} else if($script["type"] == "js" && (in_array("all", $script["page"]) || in_array($currentPageFile, $script["page"]))){
					?><script src="<?php echo $script["url"]; ?>"></script><?php 
    ...
    public function loginFooterIncludes() {
		if(sizeof($this->loginFooter)>0){
			$currentPage=explode("/",$_SERVER['SCRIPT_NAME']);
			$currentPageFile = end($currentPage);
			foreach($this->loginFooter as $script ){
				if($script["type"] == "css" && (in_array("all", $script["page"]) || in_array($currentPageFile, $script["page"]))){
					?><link rel="stylesheet" href="<?php echo $script["url"]; ?>"><?php 
				} else if($script["type"] == "js" && (in_array("all", $script["page"]) || in_array($currentPageFile, $script["page"]))){
					?><script src="<?php echo $script["url"]; ?>"></script><?php 
	...
	public function forgetpwdHeaderIncludes() {
		if(sizeof($this->forgetpwdHeader)>0){
			$currentPage=explode("/",$_SERVER['SCRIPT_NAME']);
			$currentPageFile = end($currentPage);
			foreach($this->forgetpwdHeader as $script ){
				if($script["type"] == "css" && (in_array("all", $script["page"]) || in_array($currentPageFile, $script["page"]))){
					?><link rel="stylesheet" href="<?php echo $script["url"]; ?>"><?php 
				} else if($script["type"] == "js" && (in_array("all", $script["page"]) || in_array($currentPageFile, $script["page"]))){
					?><script src="<?php echo $script["url"]; ?>"></script><?php 
    ...
    public function forgetpwdFooterIncludes() {
		if(sizeof($this->forgetpwdFooter)>0){
			$currentPage=explode("/",$_SERVER['SCRIPT_NAME']);
			$currentPageFile = end($currentPage);
			foreach($this->forgetpwdFooter as $script ){
				if($script["type"] == "css" && (in_array("all", $script["page"]) || in_array($currentPageFile, $script["page"]))){
					?><link rel="stylesheet" href="<?php echo $script["url"]; ?>"><?php 
				} else if($script["type"] == "js" && (in_array("all", $script["page"]) || in_array($currentPageFile, $script["page"]))){
					?><script src="<?php echo $script["url"]; ?>"></script><?php 
	...
	public function resetpwdHeaderIncludes() {
		if(sizeof($this->resetpwdHeader)>0){
			$currentPage=explode("/",$_SERVER['SCRIPT_NAME']);
			$currentPageFile = end($currentPage);
			foreach($this->resetpwdHeader as $script ){
				if($script["type"] == "css" && (in_array("all", $script["page"]) || in_array($currentPageFile, $script["page"]))){
					?><link rel="stylesheet" href="<?php echo $script["url"]; ?>"><?php 
				} else if($script["type"] == "js" && (in_array("all", $script["page"]) || in_array($currentPageFile, $script["page"]))){
					?><script src="<?php echo $script["url"]; ?>"></script><?php 
    ...
    public function resetpwdFooterIncludes() {
		if(sizeof($this->resetpwdFooter)>0){
			$currentPage=explode("/",$_SERVER['SCRIPT_NAME']);
			$currentPageFile = end($currentPage);
			foreach($this->resetpwdFooter as $script ){
				if($script["type"] == "css" && (in_array("all", $script["page"]) || in_array($currentPageFile, $script["page"]))){
					?><link rel="stylesheet" href="<?php echo $script["url"]; ?>"><?php 
				} else if($script["type"] == "js" && (in_array("all", $script["page"]) || in_array($currentPageFile, $script["page"]))){
					?><script src="<?php echo $script["url"]; ?>"></script><?php 
